...
class VGGBase(nn.Module):
    ...
    def __init__(self):
        ...
        self.pool5 = nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)  # retains size because stride is 1 (and padding)
    ...
    def forward(self, image):
        ...
        out = F.relu(self.conv1_1_bn(self.conv1_1(image)))
        print(out.shape)
        out = F.relu(self.conv1_2_bn(self.conv1_2(out)))
        print(out.shape)
        out = self.pool1(out)  
        print(out.shape)
        out = F.relu(self.conv2_1_bn(self.conv2_1(out)))
        print(out.shape)
        out = F.relu(self.conv2_2_bn(self.conv2_2(out)))
        print(out.shape)
        out = self.pool2(out) 
        print(out.shape)
        out = F.relu(self.conv3_1_bn(self.conv3_1(out)))
        print(out.shape)
        out = F.relu(self.conv3_2_bn(self.conv3_2(out)))
        print(out.shape)
        out = F.relu(self.conv3_3_bn(self.conv3_3(out)))
        print(out.shape)
        out = self.pool3(out)
        print(out.shape)
    ...
    def load_pretrained_layers(self):
        ...
        pretrained_feat = torchvision.models.vgg16_bn(pretrained=True).features
        pretrained_param_names = list(pretrained_state_dict.keys())
        pretrained_feat[0].state_dict()
        
        # Transfer conv. parameters from pretrained model to current model
        state_dict[param_names[0]]
        for i, param in enumerate(param_names[:-18]):  # excluding conv6 and conv7 parameters            
            if param == 'conv1_1.weight':
                state_dict[param] = pretrained_state_dict[pretrained_param_names[i]][:, 0:3, :, :]              
            else:
        ...
        # state_dict['conv6.weight'] = decimate(conv_fc6_weight, m=[8, None, 3, 3])  
        # state_dict['conv6.bias'] = decimate(conv_fc6_bias, m=[8])  
        # # fc7
        # conv_fc7_weight = pretrained_state_dict['classifier.3.weight'].view(4096, 4096, 1, 1) 
        # conv_fc7_bias = pretrained_state_dict['classifier.3.bias']  
        # state_dict['conv7.weight'] = decimate(conv_fc7_weight, m=[8, 8, None, None]) 
        # state_dict['conv7.bias'] = decimate(conv_fc7_bias, m=[8])  
