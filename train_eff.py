...
from model_eff8 import SSD300, MultiBoxLoss
...
#         optimizer = torch.optim.RMSprop(params=[{'params': biases, 'lr': 2 * lr}, {'params': not_biases}], lr=lr, momentum=momentum, weight_decay=weight_decay)
    ...
    transforms2 = Compose([ Resize(),
                            RandomHorizontalFlip(), \
                            RandomResizedCrop( [1024,1280], scale=(0.25, 4.0), ratio=(0.8, 1.2)), \
                            ToTensor(), \
                            Normalize( [0.3465,0.3219,0.2842], [0.2358,0.2265,0.2274], 'R')
                            ])
...
#                                                 pin_memory=True)
    ...
    train_dataset = KAISTPed('for-train-ho.txt',img_transform=preprocess2, co_transform=transforms2)
    ...
    val_dataset = KAISTPed('for-val-ho.txt',img_transform=preprocess1, co_transform=transforms1)
