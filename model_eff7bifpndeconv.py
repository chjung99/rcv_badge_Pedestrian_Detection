...
#         return F.upsample(x, size=(H,W), mode='bilinear') + y
    ...
    def forward(self, image):
        w1 = F.relu(self.w1)
        w1_2=F.relu(self.w1_2)
        
        w2 = F.relu(self.w2)
        w2_2=F.relu(self.w2_2)
        
        w3 = F.relu(self.w3)
        w3_2=F.relu(self.w3_2)
        
        w4 = F.relu(self.w4)
        w4_2=F.relu(self.w4_2)
        ...
        feat5=((w4*cfeat6)+(w4_2*features[5]))/(w4+w4_2+self.eps)
        cfeat5=(self.deconv5_bn(self.deconv5(feat5)))
        
        feat4=((w3*cfeat5)+(w3_2*features[4]))/(w3+w3_2+self.eps)
        cfeat4=(self.deconv4_bn(self.deconv4(feat4)))
        
        feat3=((w2*cfeat4)+(w2_2*features[3]))/(w2+w2_2+self.eps)
        cfeat3=(self.deconv3_bn(self.deconv3(feat3)))
        
        feat2=((w1*cfeat3)+(w1_2*features[2]))/(w1+w1_2+self.eps)
