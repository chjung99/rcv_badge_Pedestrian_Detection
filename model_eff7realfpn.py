...
class VGGBase(nn.Module):
    ...
    def __init__(self, version=0):
        ...
        self.secdech=nn.Conv2d(272,512,kernel_size=3,padding=1,bias=True)
        self.secdech_bn= nn.BatchNorm2d(512, affine=True)
        
        self.topdech=nn.Conv2d(448,512,kernel_size=3,padding=1,bias=True)
        self.topdech_bn= nn.BatchNorm2d(512, affine=True)
...
#         self.deconv3_bn= nn.BatchNorm2d(56, affine=True)
    ...
    def forward(self, image):
        ...
        feat2= F.relu(self.conv1_bn(self.conv1(features[2])))
        feat3= F.relu(self.conv2_bn(self.conv2(features[3])))
        feat4= F.relu(self.conv3_bn(self.conv3(features[4])))
        feat5=F.relu(self.secdech_bn(self.secdech(features[5])))
        feat6=F.relu(self.topdech_bn(self.topdech(features[6])))
        
        cfeat5=self._upsample_add(feat6,feat5)
        cfeat4=self._upsample_add(cfeat5,feat4)
        cfeat3=self._upsample_add(cfeat4,feat3)
        cfeat2=self._upsample_add(cfeat3,feat2)
#         feat5=self._upsample_add(cfeat6,features[5])
#         cfeat5=F.relu(self.deconv5_bn(self.deconv5(feat5)))
        
#         feat4=self._upsample_add(cfeat5,features[4])
#         cfeat4=F.relu(self.deconv4_bn(self.deconv4(feat4)))
        
#         feat3=self._upsample_add(cfeat4,features[3])
#         cfeat3=F.relu(self.deconv3_bn(self.deconv3(feat3)))
        
#         feat2=self._upsample_add(cfeat3,features[2])
        

        
        return cfeat2,cfeat3,cfeat4
